...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] werd geprioriteerd en geplaatst %d/%d in Wachtrij", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] was geplaatst %d/%d in Wachtrij", tmp.name, ids[1], _pos, queueCount))
    end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if not ids then
        ...
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", kon geen van hun ID's achterhalen")
        return
    end
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Je Bent geblokkeerd tijdens het joinen!")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] werd geblokkeerd voor deelname; Reden: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] Is de Stad aan het joinen")
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] Is verwijderd van de Wachtrij van wege een Timeout!")
            else
                i = i + 1
            end
        end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [Stad Wachtrij] Stopping Hardcap... ^7")
...
commands.removeq = function(args)
    ...
    Queue:DebugPrint("VERWIJDERD " .. tostring(name) .. " UIT DE WACHTRIJ")
end

commands.printq = function()
    Queue:DebugPrint("HUIDIGE WACHTRIJLIJST")
...
commands.printc = function()
    Queue:DebugPrint("HUIDIGE VERBINDINGSLIJST")
...
commands.printcount = function()
    Queue:DebugPrint("Aantal spelers: " .. Queue:GetPlayerCount())
end
...
print("^8 esx_stadwachtrij Made By Foxi Development")
print("^8 esx_stadwachtrij Made By Foxi Development")
print("^8 esx_stadwachtrij Made By Foxi Development")